...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const mapped = data.map(e => ({ id: e.id, name: e.name, cnpj: e.cnpj, location: e.location, hasStockAccess: e.has_stock_access, active: e.active, logoUrl: e.logo_url }));
    ...
    if (req.method === 'POST') {
      ...
      await sql`
        ...
        ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, cnpj=EXCLUDED.cnpj, location=EXCLUDED.location, has_stock_access=EXCLUDED.has_stock_access, active=EXCLUDED.active, logo_url=EXCLUDED.logo_url
      `;
